...
test("saved-character metrics increment on success", async () => {
  ...
  assert.equal(snapshot.session.charsSmartEdit, 7);
...
test("stats output table shows saved rate", () => {
  const text = formatSmartEditStats({
    session: { attempts: 1, failures: 0, failureRate: 0, charsSmartEdit: 39, charsNormalEdit: 100, charsSaved: 61, charsUsedRate: 0.39, charsSavedRate: 0.61 },
    global: { attempts: 1, failures: 0, failureRate: 0, charsSmartEdit: 39, charsNormalEdit: 100, charsSaved: 61, charsUsedRate: 0.39, charsSavedRate: 0.61 }
  });
...
test("global metrics persist across extension reload", async () => {
  ...
  const second = new SmartEditMetricsStore(metricsPath);
